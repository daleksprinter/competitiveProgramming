...
double dfs(int i, int p, int rest){
    ...
    if(dp[i][p][rest] > -0.9) return dp[i][p][rest];

    double ans = 0;

    if(i == 0) return dfs(i + 1, p + 2, 0);
    
    //tonikaku suwaritai
    double tmp = 0;
    if(rest > 0){
        tmp = dfs(i + 1, p, rest - 1);
    }else{
        if(p <= m) tmp += dfs(i + 1, p + 1, 0);
        else tmp += dfs(i + 1, p, rest);
    }

    ans += tmp * (double)arr[i] / 100.0;

    tmp = 0.0;
    //dekireba suwaritai
    if(p < m){
        tmp += dfs(i + 1, p + 2, rest + 1);
    }else{
        tmp += dfs(i + 1, p, rest);
    }

    ans += tmp * (double)(100 - arr[i]) / 100.0;

    return dp[i][p][rest] = ans;
