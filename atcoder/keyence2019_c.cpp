...
signed main(){
    ...
    vi a; rep(i,0,n) a.push_back(input());
    vi b; rep(i,0,n) b.push_back(input());
    ...
    sort(all(plus)); reverse(all(plus));

    cumsum1d cs = cumsum1d(1);
    cs.init(plus);
    cs.build();

    ans += lower_bound(all(cs.arr), need) - cs.arr.begin() + 1
    ;
