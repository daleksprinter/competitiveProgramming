...
void in(){
    cin >> x >> y >> z >> k;
    rep(i,0,x) a.push_back(input());
    rep(i,0,y) b.push_back(input());
    rep(i,0,z) c.push_back(input());
}
...
void solve(){
    ...
    vi tmp;
    rep(i,0,x){
        rep(j,0,y) tmp.push_back(a[i] + b[j]);
    }

    sort(all(tmp));
    reverse(all(tmp));
    tmp = cut(tmp, 0, min(3001LL, sz(tmp)));

    vi ret;
    rep(i,0,sz(tmp)){
        rep(j,0,z) ret.push_back(tmp[i] + c[j]);
    }
    sort(all(ret));
    reverse(all(ret));
    rep(i,0,k){
