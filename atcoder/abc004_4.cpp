...
int cost(int pos, int rest){
    int ret;
    if(rest > g + b) ret = abs(700 - pos);
    else if(rest > b) ret = abs(800 - pos);
    else ret = abs(900 - pos);
    return ret;
}
...
int dfs(int pos, int rest){
    ...
    if(pos == 1300) return inf;

    int ans = min(dfs(pos + 1, rest), cost(pos, rest) + dfs(pos + 1, rest - 1));
