...
signed main(){
    ...
    rep(i,0,n){
        ...
        int win = distance(sorted.begin(), lower_bound(all(sorted), rate[i]));
        int lose = distance(upper_bound(all(sorted), rate[i]), sorted.end());

        win += hands[rate[i]][(hand[i] + 1) % 3];
        lose += hands[rate[i]][(hand[i] + 3 - 1) % 3];
        cout << win << " " << lose << " " << hands[rate[i]][hand[i]] - 1 << endl;
