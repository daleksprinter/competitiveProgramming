...
void in(){
    cin >> n >> m;
    rep(i,0,n) dist.push_back(input());
    rep(i,0,m) cost.push_back(input());
}

int dp[1010][1010];
int dfs(int pos, int day){
    ...
    if(dp[pos][day] > -1) return dp[pos][day];
    int ans = inf;
    chmin(ans, dist[pos] * cost[day] + dfs(pos + 1, day + 1)); // go
    chmin(ans, dfs(pos, day + 1));

    return dp[pos][day] = ans;
}
