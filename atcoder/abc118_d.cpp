...
int n, m;
si arr;
int need[10] = {-1, 2, 5, 5, 4, 5, 6, 3, 7, 6};
void in(){
    cin >> n >> m;
    rep(i,0,m) arr.insert(input());
}
...
string dfs(int rest){
    ...
    rep(i,1,10){
        if(exist(arr, i)){
            string tmp = dfs(rest - need[i]);
            if(tmp.back() == '$') continue;
            if(isbig(ans, to_string(i) + tmp)){
                ans = to_string(i) + tmp;
            }
        }
    }
...
void solve(){
    ...
    rep(i,0,10010) dp[i] = "-1";
    string ans = dfs(n);
    print(cut(ans, 0, sz(ans) - 1));
