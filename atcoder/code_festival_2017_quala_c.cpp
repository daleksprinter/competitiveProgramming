...
void solve(){
    ...
    if(h % 2 == 1 && w % 2 == 1){
        bool isok = true;
        if(mods[3]) isok = false;
        if(mods[1] != 1) isok = false;
        if(mods[2] > (h - 1)/2 + (w - 1)/2) isok = false;
    ...
    int even = (h % 2 == 0 ? h : w);
    bool isok = true;
    if(mods[1] or mods[3]) isok = false;
    if(mods[2] > even / 2) isok = false;
