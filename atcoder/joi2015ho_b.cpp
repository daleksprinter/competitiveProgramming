...
//0... joi 1...ioi
int getnxtl(int left){
    return (left - 1 + n) % n;
}

int getnxtr(int right){
    return (right + 1) % n;
}
...
int dfs(int left, int right, int turn){
    ...
    if(left == right) return (turn == 0 ? arr[left] : 0);

    if(dp[left][right][turn] > -1) return dp[left][right][turn];

    int nxtl = getnxtl(left);
    int nxtr = getnxtr(right);
    int nxtturn = turn ^ 1;

    if(turn == 1){//enemy turn
        if(arr[left] > arr[right]){
            return dfs(nxtl, right, nxtturn);
        }else{
            return dfs(left, nxtr, nxtturn);
        }
    }else{//player trun
        int ans = 0;
        chmax(ans, arr[left] + dfs(nxtl, right, nxtturn));
        chmax(ans, arr[right] + dfs(left, nxtr, nxtturn));
...
void solve(){
    ...
    int ans = 0;
    rep(i,0,n){
        chmax(ans, arr[i] + dfs(getnxtl(i), getnxtr(i), 1));
    }
    print(ans);
