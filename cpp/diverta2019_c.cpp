...
void solve(){
    ...
    rep(i,0,n){
        if(arr[i].front() == 'B' && arr[i].back() == 'A') cnttmp++;
        else if(arr[i].back() == 'A') cntenda++;
        else if(arr[i].front() == 'B') cntendb++;
        rep(j,0,sz(arr[i]) - 1){
    ...
    print(ans + min(cntenda, cntendb) + max(0LL,cnttmp - 1) + (cnttmp > 0LL && max(cntenda,cntendb) > 0LL));
