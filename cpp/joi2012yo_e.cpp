...
const int dxodd[6]={1,0,-1,0,-1,-1};
const int dyodd[6]={0,1,0,-1,-1,1};
const int dxeven[6]={1,0,-1,0,1,1};
const int dyeven[6]={0,1,0,-1,-1,1};
void dfs(int y, int x){
    ...
    if(y % 2 == 1){
        rep(i,0,6){
            int ty = y + dyeven[i];
            int tx = x + dxeven[i];
            if(!isin(ty, 0, 105) or !isin(tx, 0, 105)) continue;
            if(arr[ty][tx]){
                ans++;
            }else{
                dfs(ty, tx);
