...
void solve(){
    ...
    rep(i,0,sz(segs)){
        ...
        int right;
        if(p == 1){
            cnt += cs1.query(i, min(sz(cnt0) - 1, i + (k * 2)));
            cnt += cs0.query(i, min(sz(cnt0) - 1, i + (k * 2)));
            right = i + k * 2;
        }else{
            cnt += cs1.query(i, min(sz(cnt0) - 1, i + k * 2 - 1));
            cnt += cs0.query(i, min(sz(cnt0) - 1, i + (k - 1) * 2));
            right = i + k * 2 - 1;
        }
//        cout << p << ' ' << i << ' ' << cnt <<' ' << min(sz(segs) - 1, right) << endl;
        p ^= 1;
        chmax(ans, cnt);
