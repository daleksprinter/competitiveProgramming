...
signed main(){
    ...
    string name = raw_input();
    transform(name.begin(), name.end(), name.begin(), ::tolower  );
    map<char, int> mpname;
    for(char e : name){
        mpname[e]++;
        /*
        if(mpname.find(e) == mpname.end()){
            mpname[e] = 0;
        }else{
            mpname[e]++;
        }
    ...
    string kit = raw_input();
    transform(kit.begin(), kit.end(), kit.begin(), ::tolower);
    map<char, int> mpkit;
    for(char e: kit){
        mpkit[e]++;
    }
    ...
    int ans = 0;
    for(char e : alpha){
        int namech = mpname[e], kitch = mpkit[e];
        if(namech == 0) continue;
