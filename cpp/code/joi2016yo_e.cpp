...
void in(){
    cin >> n >> m >> k >> s;
    cin >> a >> b;
    rep(i,0,k) zombi.push_back(input() - 1);
    rep(i,0,m){
        int f, t;
        cin >> f >> t;
        f--; t--;
...
void solve(){
    ...
    rep(i,0,k) que.push(pi(zombi[i], 0));
    ...
    while(not que.empty()){
        ...
        if(distz[v] < d) continue;

        distz[v] = d;

        rep(i,0,sz(edges[v])){
            int nxt = edges[v][i];
            if(distz[nxt] > d + 1) que.push(pi(nxt, d + 1));
        }
    }
    ...
    while(not q.empty()){
        ...
        rep(i,0,sz(edges[v])){
            int nxt = edges[v][i];
            if(distz[nxt] == 0) continue;
            int cost = (distz[nxt] <= s ? b : a);
            if(chmin(dist[nxt], d + cost)) q.push(pi(dist[nxt], nxt));
        }

    }

    print(dist[n - 1] - (distz[n - 1] <= s ? b : a));
