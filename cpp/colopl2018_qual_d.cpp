...
void in(){
    ...
    rep(i,0,n){
        auto it = lower_bound(all(arr), arr[i] + x);
        nxt1.push_back(it - arr.begin());
        if((it - 1) - arr.begin() != i) it--;
        nxt0.push_back(it - arr.begin());
...
int dfs(int i, int k){
    ...
    int ans = 0;
    chmax(ans, min(x, arr[nxt0[i]] - arr[i]) + dfs(nxt0[i], k - 1));
    chmax(ans, min(x, arr[nxt1[i]] - arr[i]) + dfs(nxt1[i], k - 1));
