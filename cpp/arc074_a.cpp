...
int divide3(int a, int b){ // divide a
    if(a % 3 == 0){
        return 0;
    }else{
        return b;
...
int dividetate(int a, int b){
    int ans = inf;
    rep(i,1,a - 1){
        int rest = a - i;
        int ret = max({i * b, b / 2 * rest, ceil(b, 2) * rest}) - min({i * b, b / 2 * rest, ceil(b, 2) * rest});
        chmin(ans, ret);
    }
    return ans;
}
...
void solve(){
    ...
    chmin(ans, divide3(n, m));
    chmin(ans, divide3(m, n));
    chmin(ans, dividetate(n, m));
    chmin(ans, dividetate(m, n));
